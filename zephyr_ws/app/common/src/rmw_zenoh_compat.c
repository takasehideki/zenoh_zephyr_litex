...
int rmw_zenoh_build_pub_liveliness(char* buf, size_t buf_len,
  ...
  int ret = snprintk(buf, buf_len,
                     "@ros2_lv/%d/%s/%d/%d/MP/%%/%%/%s/%%chatter/%s/%s/%s",
                     RMW_ZENOH_DOMAIN_ID, session_zid, node_id, pub_id,
                     node_name, RMW_ZENOH_STRING_TYPE_NAME,
                     RMW_ZENOH_STRING_TYPE_HASH, RMW_ZENOH_PUB_QOS_KEY);
  return (ret < 0 || (size_t)ret >= buf_len) ? -ENOMEM : 0;
}
...
int rmw_zenoh_build_sub_liveliness(char* buf, size_t buf_len,
  ...
  int ret = snprintk(buf, buf_len,
                     "@ros2_lv/%d/%s/%d/%d/MS/%%/%%/%s/%%chatter/%s/%s/%s",
                     RMW_ZENOH_DOMAIN_ID, session_zid, node_id, sub_id,
                     node_name, RMW_ZENOH_STRING_TYPE_NAME,
                     RMW_ZENOH_STRING_TYPE_HASH, RMW_ZENOH_SUB_QOS_KEY);
  return (ret < 0 || (size_t)ret >= buf_len) ? -ENOMEM : 0;
}
